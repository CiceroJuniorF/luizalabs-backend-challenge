...
async def test_should_add_product_to_favorites(mock_customer_repository: CustomerRepository, 
    ...
    mock_favorite_product_repository.exists_product_in_customer_favorites.assert_called_with(CUSTOMER_ID, PRODUCT_ID)
    mock_id_generator.generate.assert_called_once()
    mock_favorite_product_repository.add.assert_called_once()
    mock_favorite_product_repository.add.assert_called_with(FavoriteProduct.from_dict(favorite_product))
